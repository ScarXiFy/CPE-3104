	...
	; TOGGLEPOWER:
	 ...
	 GETSENSOR:
	    ...
	    CHECKALARM:
	       MOV AL, PH
	       CMP AL, PHMAX
	       JLE CHECK_PH_LOW		;IF PH <= MAX THEN CHECK IF LOW
	  ...
	  REFRESH_T_OFFON:
	       CMP HEATING_FLAG, 1
	       JE DISP_HEATINGON
	       MOV AL, 90H			;display heating off
	       CALL INST_CTRL
	       CALL DISPOFF
	       JMP CHECKDISP_COOLING
	       
	       DISP_HEATINGON:		;display heating on
	       MOV AL, 90H
	       CALL INST_CTRL
	       CALL DISPON
	       ...
	       CMP COOLING_FLAG, 1
	       JE DISP_COOLINGON
	       MOV AL, 0D0H		;display cooling off
	       CALL INST_CTRL
	       CALL DISPOFF
	       JMP READ_KEY_TEMP
	       
	       DISP_COOLINGON:
	       MOV AL, 0D0H		;display cooling on
	       CALL INST_CTRL
	       CALL DISPON
	       
	       ;==================================================NEED TO ADD SOMETHING HERE TO CHECK IF HEATING/COOLING ARE ALREADY ON OR NOT THEN DISPLAY ON/OFF
		  ...
		  HEATTOGGLE:
		     CMP HEATING_FLAG, 0	;IF HEATING CURRENTLY OFF THEN ON
		     JE SET_HEAT_ON
		     MOV HEATING_FLAG, 0	;IF HEATING CURRENTLY ON THEN OFF
		     JMP HEAT_OFF
		  ...
		  COOLTOGGLE:
		     CMP COOLING_FLAG, 0	;IF 0 COOLING THEN 1 COOLING
		     JE SET_COOL_ON
		     MOV COOLING_FLAG, 0	;IF 1 COOLING THEN 0 COOLING
		  COOL_OFF:
		     MOV DX, PORTB
		     MOV AL, 0F7H
		     AND AL, ENABLE_PORTS
		  ...
		  ; =========== PH CONTROL MENU ==================
	       ...
	       DISP_ACIDON:		;display heating on
	       MOV AL, 90H
	       CALL INST_CTRL
	       CALL DISPON
	       ...
	       CALL INST_CTRL
	       CALL DISPON
	       ;==================================================NEED TO ADD SOMETHING HERE TO CHECK IF ACID/ALK ARE ALREADY ON OR NOT THEN DISPLAY ON/OFF
...
; =========================== SETTING THRESHOLD ======================================================================= 
		 ...
		 XOR AX,AX
		  MOV DX, PORTC
		  IN AL, DX
		  TEST AL, 10H            ; if 10000 -> a key is pressed, da = 1
		  JZ CHECK_DAVBL
		  ...
		  SET_MIN:
		     ...
		     CMP TEMPTHRMENU_FLAG, 1
		     JNE CHECK_PH_MIN
					  ;IF IN TEMP THRESH, THEN SET TEMP THRESH MIN 
		     MOV AL, COMBINEDNUM
		     CMP AL, 40
		     JL VALID_TEMP_MIN
		  ...
		  VALID_TEMP_MIN:
		     MOV TEMPMIN_HOLDER, AL
		     JMP CLEAR_AND_CONTINUE
					  ;IF IN PH THRESH, THEN SET PH THRESH MIN 
		  CHECK_PH_MIN:
		     CMP PHTHRMENU_FLAG, 1
		  ...
		  SET_MAX:
		     ...
		     MOV AL, COMBINEDNUM
		     CMP AL, 0
		     JLE INVALID_TEMP_MAX		;IF MAX <= 0, INVALID
		     CMP AL, 40
		     JG INVALID_TEMP_MAX		;IF MAX >40, INVALID
		     CMP AL, TEMPMIN_HOLDER
		     JGE VALID_TEMP_MAX			;IF MAX > MIN, VALID
		  ...
		  CHECK_PH_MAX:
		     MOV AL, COMBINEDNUM
		     CMP AL, PHMIN_HOLDER
		     JL INVALID_PH_MAX	;IF PHMAX < PHMIN THEN INVALID
		     CMP AL, 14
		     JLE VALID_PH_MAX	;IF PHMAX <= 14 THEN VALID
		  ...
		  CLEAR_AND_CONTINUE:
		     ...
		     MOV COMBINEDNUM, 0
		     mov al, 99h
		     CALL INST_CTRL		;SETUP CURSOR FOR MAX VALUE
		     JMP CHECK_DAVBL
 ...
 ; =======================MENU FOR VIEW THRESH VALUES ======================================================================= 
	...
	CHECK_VIEWTHRMENU_CHOICE:
	    ...
	    MOV DX, PORTC
	    IN AL, DX
	    AND AL, 0FH		; NOT SURE IF 10H OR 0FH
